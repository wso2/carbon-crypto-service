...
public class DefaultCryptoService implements CryptoService, PrivateKeyRetriever {
    ...
    public byte[] sign(byte[] data, String algorithm, String javaSecurityAPIProvider, CryptoContext cryptoContext)
        ...
        if (areExternalCryptoProvidersAvailable()) {
            ...
            return mostSuitableExternalProvider.sign(data, algorithm, javaSecurityAPIProvider,
    ...
    public byte[] decrypt(byte[] ciphertext, String algorithm, String javaSecurityAPIProvider,
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Decrypting data using the algorithm '%s' and the " +
                    "Java Security API provider '%s'; %s", algorithm, javaSecurityAPIProvider, cryptoContext));
        }
        ...
        if (areExternalCryptoProvidersAvailable()) {
            ...
            return mostSuitableExternalProvider.decrypt(ciphertext, algorithm, javaSecurityAPIProvider,
                    cryptoContext, privateKeyInfo);
        } else {
            String errorMessage = String.format("No external crypto providers available. Correctly register " +
                    "a service implementation of '%s' as an OSGi service", ExternalCryptoProvider.class);
    ...
    public byte[] encrypt(byte[] cleartext, String algorithm, String javaSecurityAPIProvider,
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Encrypting data using the algorithm '%s' and the " +
                    "Java Security API provider '%s'; %s", algorithm, javaSecurityAPIProvider, cryptoContext));
        }
        ...
        if (areExternalCryptoProvidersAvailable()) {
            ...
            return mostSuitableExternalProvider.encrypt(cleartext, algorithm, javaSecurityAPIProvider,
    ...
    public boolean verifySignature(byte[] data, byte[] signature, String algorithm, String javaSecurityAPIProvider,
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Verifying the signature using the algorithm '%s' and the " +
                    "Java Security API provider '%s'; %s", algorithm, javaSecurityAPIProvider, cryptoContext));
        }
        ...
        if (areExternalCryptoProvidersAvailable()) {
            ...
            return mostSuitableExternalProvider.verifySignature(data, signature, algorithm, javaSecurityAPIProvider,
                    cryptoContext, certificateInfo);
        } else {
            String errorMessage = String.format("No external crypto providers available. Correctly register " +
                    "a service implementation of '%s' as an OSGi service", ExternalCryptoProvider.class);
    ...
    /**
     * Computes and returns the ciphertext of the given cleartext using the given internal crypto provider.
     *
     * @param cleartext                     The cleartext to be encrypted.
     * @param javaSecurityAPIProvider       The Java Security API provider.
     * @param returnSelfContainedCipherText Whether cipher text need to be self contained.
     ...
     */
    @Override
    public byte[] encryptWithInternalProviderType(byte[] cleartext, String javaSecurityAPIProvider,
                                                  boolean returnSelfContainedCipherText,
                                                  String internalCryptoProviderType)
            throws CryptoException {

        failIfInternalCryptoInputsAreNotValid(cleartext, internalCryptoProviderType);
        if (log.isDebugEnabled()) {
        ...
        if (areInternalCryptoProvidersAvailable()) {
            ...
            return mostSuitableInternalProvider
                    .encrypt(cleartext, javaSecurityAPIProvider, returnSelfContainedCipherText);
    ...
    public byte[] decryptWithInternalProviderType(byte[] ciphertext, String javaSecurityAPIProvider,
                                                  String internalCryptoProviderType) throws CryptoException {

        failIfInternalCryptoInputsAreNotValid(ciphertext, internalCryptoProviderType);
        ...
        if (areInternalCryptoProvidersAvailable()) {
            ...
            return mostSuitableInternalProvider.decrypt(ciphertext, javaSecurityAPIProvider);
        } else {
            String errorMessage = String.format("No internal crypto providers available. Correctly register " +
                    "a service implementation of '%s' as an OSGi service", InternalCryptoProvider.class);
            throw new CryptoException(errorMessage);
    ...
    public InternalCryptoProvider getMostSuitableInternalProvider() throws CryptoException {
        ...
        if (isInternalCryptoProviderConfiguredInConfigFile()) {
            ...
            if (internalCryptoProviders.isEmpty()) {
                ...
                mostSuitableProvider = ((Map.Entry<String, InternalCryptoProvider>) (internalCryptoProviders
     ...
     * @throws CryptoException
     */
    public InternalCryptoProvider getMostSuitableInternalProvider(String internalCryptoProviderType)
            throws CryptoException {
    ...
    public void overrideKeyResolverPriority(String keyResolverClassName, int keyResolverPriority) {
        ...
        for (KeyResolver keyResolver : keyResolvers) {
            if (keyResolver.getClass().getCanonicalName().equals(keyResolverClassName)) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug(String.format("Setting %d as the priority of the key resolver '%s'",
                            keyResolverPriority, keyResolverClassName));
                }
        ...
        for (KeyResolver privateKeyResolver : keyResolvers) {
            if (privateKeyResolver.isApplicable(cryptoContext)) {
                ...
                if (privateKeyInfo != null) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug(String.format("Private key info was not returned by %s. " +
                                "Continuing with remaining resolvers.", privateKeyResolver));
                    }
        ...
        for (KeyResolver privateKeyResolver : keyResolvers) {
            if (privateKeyResolver.isApplicable(cryptoContext)) {
                ...
                if (certificateInfo != null) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug(String.format("Certificate info was not returned by %s. " +
                                "Continuing with remaining resolvers.", privateKeyResolver));
                    }
    ...
    private void failIfInternalCryptoInputsAreNotValid(byte[] data, String internalCryptoProviderType)
            throws CryptoException {

        if (data == null) {
            throw new CryptoException("Content provided can't be null");
        }
